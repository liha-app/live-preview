...
export const ja: Messages = {
  ...
  'app.tagline':
    'ビルド成果物・デザイン・ドキュメントを固定URLで共有。レビュアーが見たままに指摘を書き込み、同じタブにいるAIエージェントがその文脈を WebMCP 経由で構造化データとして読み取り、修正版は同じURLに届きます。',
  ...
  'home.urlHint':
    'ページをスナップショットして、その上に指摘を書き込めるようにします。レイアウトと文言はそのまま来ますが、Webフォントとモジュールスクリプトは多くの場合来ません（サイト側が他オリジンへの利用を許可している必要があります）。自分のビルドなら、ドロップかCLIのほうが何も失いません。プライベートIPや内部アドレスは拒否されます。',
